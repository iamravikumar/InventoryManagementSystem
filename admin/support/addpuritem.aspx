...
<%
    try
    {
        ...
        if(Request["item_qty_type"]!=null)
            validator.Add("item_qty_type", true, InputRule.DIGITS);
        if(Request["stkitem_qty"]!=null)
            validator.Add("stkitem_qty", true, InputRule.DECIMAL);
        ...
        if (validator.Validate())
        {
            ...
            {
                string itmqtyType = Request["item_qty_type"];
                if(string.IsNullOrWhiteSpace(itmqtyType))
                {
                    itmqtyType = new ClassDB(Request, Response).GetText("select per_unit_qty from tbl_item_master where company_id='" + new ClassDB(Request, Response).COMPANY["id"] + "' and id='" + Request["item_name"] + "'");
                }

                string stkitem_qty = Request["stkitem_qty"];
                if(string.IsNullOrWhiteSpace(stkitem_qty))
                {
                    stkitem_qty = Request["item_qty"];
                }
                dtItem.Rows.Add(Request["item_name"], Request["item_qty"], itmqtyType, stkitem_qty,
