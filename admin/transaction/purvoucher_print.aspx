        ...
        <div class="box box-default">
            <div class="box-header with-border">
                ...
                <div class="box-tools pull-right">
                    <a href="purvoucher_list.aspx?cmd=Clear" class="btn btn-primary"><i class="fa fa-fw fa-long-arrow-left"></i>Back</a>
                </div>
            </div>
            <%inventory.Execute();

                if(inventory.TempRs["invoice_type"]=="PO")
                {
                    Response.Redirect("order_print.aspx?uid="+ inventory.MD5(inventory.TempRs["id"]) +"&wid=eccbc87e4b5ce2fe28308fd9f2a7baf3");
                }
            ...
            <section class="invoice">
                ...
                <div id="printableArea">
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><%=rdr["rate"]%></td>
                                        <td><%=rdr["net_rate"]%></td>
                                        <td><%=rdr["disc_amt"]%> (<%=rdr["disc_per"]%> %)</td>
                                        <td><%=rdr["gross_amt"]%></td>
                                        <td><%=rdr["gst_amt"]%> (<%=rdr["gst_per"]%> %)</td>
                                        <td><%=rdr["cess_amt"]%> (<%=rdr["cess_per"]%> %)</td>
                                        <td><%=rdr["total_rate"]%></td>
                                    </tr>
                                    <%} rdr.Close();%>
                                    <tr>
                                        <td colspan="10"><b>* Rate With <%=inventory.TempRs["taxinclude"].ToString()=="1"?"Tax Included":"Tax Excluded" %>.</b></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-xs-6">
                            *<%=inventory.GetText("tbl_invoice_setup_master", "footer_text", "invoice_type='PURCHASE' and company_id='" + inventory.COMPANY["id"] + "' and effective_date<=getdate() and status=1 order by effective_date desc") %>
                        </div>
                    </div>
                </div>
                <!-- .print row -->
                ...
                <div class="row no-print">
                    <div class="col-xs-12" style="text-align:center">
                        <button type="button" class="btn btn-success" style="margin-right: 8px;" onclick="printDiv('printableArea')">
                            <i class="fa fa-print"></i>Print Invoice
                        </button>
                        <button type="button" class="btn btn-info" style="margin-right: 8px;" onclick="printDiv('printableArea')">
