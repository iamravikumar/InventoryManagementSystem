    ...
    function getDet(e) {
        ...
        xmlhttp.onreadystatechange = function () {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                ...
                $("#rate").val(data.purchase_rate).keyup();
                $("#discper").val("0").keyup();
                $("#gross").val(data.purchase_rate).keyup();
        ...
        <div class="box box-default">
            <div class="box-header with-border">
                ...
                <div class="box-tools pull-right">
                    <a href="purchaseret_list.aspx?cmd=Clear" class="btn btn-primary"><i class="fa fa-fw fa-long-arrow-left"></i>Go To List</a>
                </div>
            </div>
            <form method="post" role="form" id="FORM1" name="FORM1">
                <div class="box-body">
                    <% inventory.Execute();
                        ...
                        if(Request["uid"]==null)
                        {
                            poInv=inventory.getInvoice("PURCHASE RETURN");
                        ...
                        {
                            if(inventory.TempRs["invoice_type"]=="P")
                            {
                                poInv=inventory.getInvoice("PURCHASE RETURN");
                                if(poInv["invType"].ToString()=="MANUAL")
                                {
                        ...
                        if (Session["dtItem"] == null)
                        {
                            ...
                            SqlDataReader dr1 = inventory.GetDataReader("select * from tbl_stock_details where ref_id='" + inventory.TempRs["id"] + "' and ref_by='tbl_inventory_master' and status=1 order by id asc");
                            while(dr1.Read())
                            {
                                string free_item=null;
                                string free_item_qty=null;
                                string free_qty_type=null;
                                Hashtable frrhs = inventory.GetRecords("select item_id, inwardqty, inwardunit from tbl_stock_details where ref_id='" + inventory.TempRs["id"] + "' and ref_by='tbl_inventory_master' and itemid_freewith='" + dr1["id"] + "' and is_free_item=1 and status=1");
                                if(frrhs.Count > 0)
                                {
                                    free_item=frrhs["item_id"].ToString();
                                    free_item_qty=frrhs["inwardqty"].ToString();
                                    free_qty_type=frrhs["inwardunit"].ToString();
                                }
                                dtItem.Rows.Add(dr1["item_id"], dr1["inwardqty"], dr1["inwardunit"], dr1["inwardunitqty"], dr1["reference_no"], dr1["mfg_dt"], dr1["exp_dt"], dr1["rate"], dr1["net_rate"], dr1["disc_per"], dr1["disc_amt"], dr1["gross_amt"], dr1["gst_per"], dr1["gst_amt"], dr1["cess_per"], dr1["cess_amt"], dr1["total_rate"], free_item, free_item_qty, free_qty_type, dr1["store_id"],inventory.TempRs["ledger_id"],inventory.TempRs["taxinclude"]);
                        ...
                        {
                            dues="CR :- &#8377; 0.00";
                            Hashtable ldue = inventory.getSupDue(lgr_id, DateTime.Now.ToString("yyyy-MM-dd"));
                            if(ldue.Count>0)
                            {
                    ...
                    <div class="form-group row">
                        <div class="col-md-7 col-sm-12">
                                ...
                                <div class="col-md-9 col-sm-7 ">
                                    <div class="col-md-11 col-sm-11" style="padding-left: 0px">
                                        <select name="supplier" id="supplier" required class="form-control input-sm" <%=disabled %> style="cursor:pointer">
                                          ...
                                          <%
                                            DataTable dtLdg = inventory.GetDataTable("select id,name,code,mobile_no, address_1 from tbl_ledger_master where company_id='" + inventory.COMPANY["id"] + "' and group_id in ('1','4') and status=1 order by id asc");
                            ...
                            <div class="form-group row">
                                <label for="invoice_no" class="col-md-3 col-sm-5 control-label">Invoice No. <span style="font-weight: bold; color: red">*</span></label>
                                <div class="col-md-6 col-sm-7">
                                    <input type="text" name="invoice_no" id="invoice_no" <%=disableonpay %> value="<%=inventory.TempRs["invoice_no"]!=""?inventory.TempRs["invoice_no"]:poInv["invNo"] %>" required class="form-control input-sm" onkeyup="this.value=this.value.toUpperCase()" />
                                </div>
                            </div>

                            <div class="form-group row">
                                <label for="invoice_dt" class="col-md-3 col-sm-5 control-label">Invoice Date <span style="font-weight: bold; color: red">*</span></label>
                                <div class="col-md-6 col-sm-7">
                                    <input type="text" name="invoice_dt" id="invoice_dt" <%=disableonpay %> required class="form-control input-sm" value="<%=inventory.TempRs["invoice_dt"]!=""?Convert.ToDateTime(inventory.TempRs["invoice_dt"]).ToString("dd-MM-yyyy"):DateTime.Now.ToString("dd-MM-yyyy") %>" />
                                </div>
                            </div>
                        ...
                        <div class="col-md-5 col-sm-12">
                            <div class="form-group row">
                                ...
                                <div class="col-md-6 col-sm-7">
                                    <input type="text" name="order_no" id="order_no" value="<%=inventory.TempRs["invoice_type"]=="P"?poInv["invNo"]:inventory.TempRs["autogen_id"]!=""?inventory.TempRs["autogen_id"]:poInv["invNo"] %>" required class="form-control input-sm" <%=read_only %> onkeyup="this.value=this.value.toUpperCase()" />
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="order_dt" class="col-md-4 col-sm-5 control-label">Order Date <span style="font-weight: bold; color: red">*</span></label>
                                <div class="col-md-6 col-sm-7">
                                    <input type="text" name="order_dt" id="order_dt" required class="form-control input-sm" value="<%=inventory.TempRs["invoice_type"]=="P"?DateTime.Now.ToString("dd-MM-yyyy"):inventory.TempRs["stock_dt"]!=""?Convert.ToDateTime(inventory.TempRs["stock_dt"]).ToString("dd-MM-yyyy"):DateTime.Now.ToString("dd-MM-yyyy") %>" <%=read_only %> />
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="due_dt" class="col-md-4 col-sm-5 control-label">Due Date <span style="font-weight: bold; color: red">*</span></label>
                                <div class="col-md-6 col-sm-7">
                                    <input type="text" name="due_dt" id="due_dt" required class="form-control input-sm" value="<%=inventory.TempRs["invoice_type"]=="P"?DateTime.Now.ToString("dd-MM-yyyy"):inventory.TempRs["due_dt"]!=""?Convert.ToDateTime(inventory.TempRs["due_dt"]).ToString("dd-MM-yyyy"):DateTime.Now.ToString("dd-MM-yyyy") %>" readonly />
                                </div>
                            </div>
                        </div>
                    </div>
